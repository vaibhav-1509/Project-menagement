  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="label" stroke={theme.muted} tickLine={false} axisLine={{ stroke: theme.grid }} />
        <YAxis allowDecimals={false} stroke={theme.muted} tickLine={false} axisLine={false} width={28} />
        <Tooltip
          cursor={{ fill: theme.grid }}
          contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8 }}
        />
        <Bar dataKey="count" name="Completed" fill={theme.accent} radius={[4, 4, 0, 0]} maxBarSize={40} />
      </BarChart>
    </ResponsiveContainer>
  )
}
